...
const composeApplicationModule = require("../src/base-app/bootstrap/composeApplication");
const productResolver = require("../src/base-app/bootstrap/productResolver");
const applicationEvents = require("../src/base-app/bootstrap/applicationEvents");
const consoleHistorySettingsStoreModule = require("../src/console/services/consoleHistorySettingsStore");
const consoleHistoryIpcController = require("../src/shell-electron/console/consoleHistoryIpcController");
const importFileControllerModule = require("../src/runtime/tabular-data/importFileController");
const importFileIpcController = require("../src/shell-electron/runtime/importFileIpcController");
const externalCallIpcController = require("../src/shell-electron/dialog-runtime/externalCallIpcController");
const productDialogRuntimeComposition = require("../src/shell-electron/dialog-runtime/productDialogRuntimeComposition");
const productDialogCompositionModule = require("../src/shell-electron/dialog-runtime/productDialogComposition");
const datasetEditorWarmCacheModule = require("../src/dataset-editor/datasetEditorWarmCache");
const datasetEditorCompositionModule = require("../src/shell-electron/dataset-editor/datasetEditorComposition");
const datasetEditorSettingsModule = require("../src/dataset-editor/datasetEditorSettings");
...
const externalWindowCompositionModule = require("../src/shell-electron/external/externalWindowComposition");
const mainWindowCompositionModule = require("../src/shell-electron/windows/mainWindowComposition");
const workspacePaneWindowComposition = require("../src/shell-electron/windows/workspacePaneWindowComposition");
const applicationShellIpcComposition = require("../src/shell-electron/windows/applicationShellIpcComposition");
const runtimeLifecycleCompositionModule = require("../src/shell-electron/lifecycle/runtimeLifecycleComposition");
const electronApplicationLifecycle = require("../src/shell-electron/lifecycle/electronApplicationLifecycle");
const workspaceQuitDialogControllerModule = require("../src/shell-electron/lifecycle/workspaceQuitDialogController");
const runtimeRecoveryDialogControllerModule = require("../src/shell-electron/lifecycle/runtimeRecoveryDialogController");
...
let mainWindowCloseApprovedForQuit = false;
const mainWindowComposition = mainWindowCompositionModule.createMainWindowComposition({
    ...
    openDeveloperTools: developerToolsEnabled,
    ...
    canCloseWindow: function () {
        return electronSmokeMode || mainWindowCloseApprovedForQuit;
    },
    requestCloseQuit: function () {
        electron.app.quit();
    }
});
...
const productAutoUpdateEnabled = function () {
    const policy = composition.product.autoUpdate || {};
    const updateNotificationsEnabled = initialSettings.notifyUpdates !== false;

    return updateNotificationsEnabled
        && Boolean(policy.url || (policy.releaseRepository && policy.releaseTag));
};
const electronUpdateService = electronUpdateServiceModule.createElectronUpdateService({
    app: electron.app,
    dialog: electron.dialog,
    productName: composition.product.name,
    enabled: productAutoUpdateEnabled(),
...
installApplicationMenu = applicationSupportWindows.installApplicationMenu;
...
const findProductDialogDefinition = function (dialogId) {
    ...
    const definition = registered || findDialogDefinitionForMenu(dialogId);
    if (!definition) {
        return definition;
    }
    const settings = settingsStorage.readEffectiveSettings(settingsStoragePaths());
    const requirements = settings.dialogRuntimeRequirements
        && typeof settings.dialogRuntimeRequirements === "object"
        ? settings.dialogRuntimeRequirements[dialogId]
        : null;
    const configuredPackages = requirements
...
datasetEditorComposition = datasetEditorCompositionModule.createDatasetEditorComposition({
    ...
    translate: translateCompositionText,
    nativeWindowIconPath: composition.nativeWindowIconPath || undefined,
    pagePath: path.join(composition.rootDir, "src/base-app/pages/datasetEditor.html"),
    showOnOpen: !electronSmokeMode,
    getZoomFactor: function () {
        return mainWindowZoomController.getZoomFactor();
    },
...
electronApplicationLifecycle.bindElectronApplicationLifecycle({
    ...
    autoStartRuntime,
    checkForUpdates: electronUpdateService.checkForUpdates,
