...
export const createDatasetViewerClient = () => {
  ...
  const updateVariable = async (
    ...
    try {
      ...
      if (patch?.measure !== undefined) payload.measure = String(patch.measure);
      if (patch?.label !== undefined) payload.label = String(patch.label);
      if (patch?.width !== undefined && Number.isFinite(Number(patch.width))) {
        payload.width = Number(patch.width);
      }
      if (patch?.decimals !== undefined && Number.isFinite(Number(patch.decimals))) {
        payload.decimals = Number(patch.decimals);
      }
      if (patch?.align !== undefined) payload.align = String(patch.align);
      if (Array.isArray(patch?.categories)) {
        payload.categories = patch.categories.map((category) => ({
          value: String(category?.value || ''),
          label: String(category?.label || ''),
          isMissing: category?.isMissing === true
        }));
      }
      if (patch?.missingRange) {
        payload.missingRange = {
          min: String(patch.missingRange.min || ''),
          max: String(patch.missingRange.max || '')
        };
      } else if (patch?.missingRange === null) {
        payload.missingRange = null;
      }
