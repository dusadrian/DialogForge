...
export interface ConsoleEditorMonacoController {
    ...
    applySettings(settings?: ConsoleEditorSettings | null): void;
    dispose(): void;
...
export const createConsoleEditorMonacoController = function(
    ...
    let settings = normalizeConsoleEditorSettings();

    const applySettings = function(
        nextSettings?: ConsoleEditorSettings | null
    ): void {
        settings = normalizeConsoleEditorSettings({
            fontFamily: nextSettings?.fontFamily ?? settings.fontFamily,
            fontSize: nextSettings?.fontSize ?? settings.fontSize,
            cursorStyle: nextSettings?.cursorStyle ?? settings.cursorStyle,
            cursorBlink: nextSettings?.cursorBlink ?? settings.cursorBlinking !== "solid",
            selectionBackground:
                nextSettings?.selectionBackground ?? settings.selectionBackground
        });

        if (monacoRef) {
            updateConsoleTheme(monacoRef, {
