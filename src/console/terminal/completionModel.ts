...
export const createCompletionModel = function(
    ...
    const parseCompletionContext = options.contextParser || getCompletionContext;
    const parsePackageRequests = options.packageRequestParser || function(): string[] {
        return [];
    };
    const knowledge = new CompletionKnowledge(
        options.initialTerminalSymbols || [],
        options.suppressedTerminalSymbols || []
    );
    ...
    const registerCommandInput = function(input: string): void {
        ...
        parsePackageRequests(input).forEach((packageName) => {
            void prefetchPackage(packageName, false);
        });
    };

    return {
        getCompletionContext: parseCompletionContext,
