...
import { colorizeConsoleCodeInto } from '../consoleSyntax';
import {
  CONSOLE_FONT_FAMILY,
  CONSOLE_FONT_SIZE,
  CONSOLE_LINE_HEIGHT
...
const INSTALL_PROGRESS_COLOR = '#d31d1d';
const STRING_COLOR = '#0a7c3c';
...
const createOutputLinesBlock = (lines: readonly string[], color = '#000000') => {
  ...
  lines.forEach((line) => {
    ...
    styleRowBase(row);
    decorateOutputLine(row, String(line ?? ''), color);
    host.appendChild(row);
  });
  return host;
};

const decorateTryingUrlLine = (
  row: HTMLElement,
  line: string
): boolean => {
  const match = line.match(/^(\s*)(trying URL )('.*')(\s*)$/);

  if (!match) {
    return false;
  }

  const [, leading, prefix, url, trailing] = match;
  ...
  prefixEl.style.color = INSTALL_PROGRESS_COLOR;
  row.appendChild(prefixEl);

  const urlEl = document.createElement('span');
  urlEl.textContent = url;
  urlEl.style.color = STRING_COLOR;
  urlEl.className = 'consoleToken-string';
...
const decorateOutputLine = (
  ...
  if (decorateTryingUrlLine(row, line)) {
    return;
  }
...
const createInputLine = (prompt: string, text: string, promptWidthChars: number) => {
  ...
  void colorizeConsoleCodeInto(codeEl, text);
