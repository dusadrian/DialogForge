...
runtime_capture_input <- function(code, parent_id, output_width = NULL) {
    ...
    if (!is.null(next_width)) {
        previous_width <- getOption("width")
        options(width = next_width)
...
runtime_collapse_stream_lines <- function(lines) {
...
runtime_emit_input_result <- function(result, code, parent_id, visible) {
    ...
    output <- runtime_collapse_stream_lines(result$output)
    stderr <- runtime_collapse_stream_lines(result$stderr)
    messages <- paste(result$messages[nzchar(result$messages)], collapse = "")
    warnings <- filter_warnings(result$warnings, code)

    if (nzchar(stderr)) {
        if (nzchar(output) || nzchar(messages) || length(warnings)) {
            stderr <- paste0(stderr, "\n")
        }

        emit_stream_event(stderr, "stderr", parent_id)
    }
...
runtime_execute_input <- function(params) {
    ...
    output_width <- runtime_output_width(params$outputWidth)
...
runtime_completion_internal_symbol <- function(value) {
    symbol <- as.character(value %||% "")

    startsWith(symbol, ".") || is.element(
        symbol,
        c(
...
runtime_completion_from_search_path <- function(prefix) {
    ...
    symbols <- symbols[!vapply(
        symbols,
        runtime_completion_internal_symbol,
        logical(1)
    )]
