...
export const resolveRuntimeSourceDir = function(rootDir: string): string {
    ...
    const sourceCandidates = [
        path.join(normalizedRoot, "dist", "src", "runtime", "providers", "r", "r-sources"),
        path.join(normalizedRoot, "src", "runtime", "providers", "r", "r-sources")
    ];
...
const createSourceLauncherExpression = function(launcherPath: string): string {
    return [
        "source(",
        JSON.stringify(launcherPath),
        ", echo = FALSE, verbose = FALSE, print.eval = FALSE)"
    ].join("");
};
