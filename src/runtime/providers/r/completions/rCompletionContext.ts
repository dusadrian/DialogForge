import { CompletionContext } from "../../../../console/terminal/completionTypes";

const DATA_MASKING_FUNCTIONS = new Set(["using", "with", "inside"]);
const PACKAGE_ARGUMENT_FUNCTIONS = new Set([
    "library",
    "require",
    "requireNamespace"
]);
...
const packageArgumentContext = function(source: string): CompletionContext | null {
    const stack: Array<{ name: string; openIndex: number }> = [];
    let quote = "";
    let escaped = false;
    ...
    if (!frame || !PACKAGE_ARGUMENT_FUNCTIONS.has(frame.name)) {
        return null;
    }

    const body = source.slice(frame.openIndex + 1);

    if (firstTopLevelComma(body) >= 0) {
        return null;
    }
    ...
    if (quoted) {
        return {
            mode: "package",
            token: String(quoted[2] || ""),
            quote: String(quoted[1] || "")
        };
    }

    const unquoted = body.match(/^\s*([A-Za-z.][A-Za-z0-9._]*)$/);
...
export const getRCompletionContext = function(line: string): CompletionContext | null {
    const source = String(line || "");
    const packageContext = packageArgumentContext(source);
