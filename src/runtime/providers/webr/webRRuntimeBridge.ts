...
import {
    createWorkspaceSnapshotFromTextEntries,
    parseWorkspaceTextSnapshot
} from "../../workspace/workspaceSnapshotText";
import {
    buildRWorkspaceTextSnapshotCommand
} from "../r/workspace/rWorkspaceSnapshotCommands";
import {
...
export const handleWebRRuntimeTransportRequest = async function(
    activeRuntime: WebR,
    request: RuntimeTransportRequest
    ...
    try {
        ...
        if (request.method === webRTransportMethods.workspaceObjects) {
            const text = await activeRuntime.evalRString(
                buildRWorkspaceTextSnapshotCommand()
            );
            const workspace = createWorkspaceSnapshotFromTextEntries(
                parseWorkspaceTextSnapshot(text),
                { providerId: "webr" }
            );
    ...
    return {
        ...
        sendRequest: async function(
            ...
            return enqueueOperation(async function(): Promise<RuntimeTransportResponse> {
                return handleWebRRuntimeTransportRequest(requireRuntime(), request);
            });
        }
    };
};
