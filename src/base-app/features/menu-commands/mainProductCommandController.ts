...
const normalizePackages = function(value: unknown): string[] {
    return Array.isArray(value)
        ? value.map(String).filter(Boolean)
        : [];
};

const resolveCommandPackages = function(
    command: EvaluatedMenuItem,
    capabilities: EvaluatedProductCapability[]
): string[] {
    const directPackages = normalizePackages(command.rPackages);

    if (directPackages.length > 0) {
        return directPackages;
    }

    const capabilityName = String(command.capability || "").trim();
    const capability = (capabilities || []).find((candidate) => {
        return candidate.capability === capabilityName;
    });

    return normalizePackages(capability?.rPackages);
};
...
export const createMainProductCommandController = function(
    ...
    const execute = async function(command: EvaluatedMenuItem): Promise<void> {
        ...
        const packages = resolveCommandPackages(
            command,
            bindings.getProductCapabilities()
        );
