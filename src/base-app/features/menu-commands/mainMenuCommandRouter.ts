...
import {
    isScriptEditorFileOpenCommand,
    isScriptEditorOpenCommand
} from "./menuCommandGroups";
...
const routeShellCommand = function(
    ...
    if (isScriptEditorFileOpenCommand(command.command)) {
        bindings.openScriptFile();
        return;
    }

    if (isScriptEditorOpenCommand(command.command)) {
