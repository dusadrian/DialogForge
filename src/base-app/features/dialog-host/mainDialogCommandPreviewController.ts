export interface MainDialogCommandPreviewControllerOptions {
    ...
    resetSizeModeOnHide?: boolean;
    usePointerResize?: boolean;
    colorize(target: HTMLElement, text: string): Promise<void>;
    copyCommand?(text: string): Promise<void> | void;
...
export const createMainDialogCommandPreviewController = function(
    ...
    const getElements = function() {
        return {
            ...
            container: options.containerSelector
                ? options.document.querySelector(options.containerSelector) as HTMLElement | null
                : options.document.getElementById("visibleCommandPanel"),
    ...
    const setHeight = function(height: number): void {
        const { root, container, pane } = getElements();
    ...
    const setVisible = function(nextVisible: boolean, text: string): void {
        ...
        if (nextVisible) {
            ...
            if (options.resetSizeModeOnHide) {
                setSizeMode("auto");
            }
        }
    ...
    const handlePointerDrag = function(event: PointerEvent): void {
        if (!dragging) {
            return;
        }
    ...
    const clearPointerDragging = function(event: PointerEvent): void {
        if (!dragging) {
            return;
        }

        const { splitter } = getElements();
    ...
    const bind = function(): void {
        ...
        copyButton?.addEventListener("click", function(event): void {
            ...
            if (latestCommand.trim()) {
                void (options.copyCommand || options.writeClipboardText)(
                    latestCommand
                );
            }
        });
        ...
        if (options.usePointerResize) {
            splitter?.addEventListener("pointerdown", function(event): void {
                event.preventDefault();
                dragging = true;
                splitter.setPointerCapture?.(event.pointerId);
            });
            splitter?.addEventListener("pointermove", handlePointerDrag);
            splitter?.addEventListener("pointerup", clearPointerDragging);
            splitter?.addEventListener("pointercancel", clearPointerDragging);
        }
        else {
