...
interface ProfileCustomJSGlobal {
  dialogForgeProfileCustomJSModules?: ProfileCustomJSModule[];
}
...
export const registerProfileCustomJSModule = function(
  ...
  if (module && typeof module === 'object') {
    registeredProfileCustomJSModules.add(module);
  }
};

const readProfileCustomJSModules = function(): ProfileCustomJSModule[] {
  const modules = Array.from(registeredProfileCustomJSModules);
  const globalModules = (globalThis as ProfileCustomJSGlobal)
    .dialogForgeProfileCustomJSModules;
...
const extendApiFromProfileModule = async (
  module: ProfileCustomJSModule,
  api: ProfileCustomJSApi,
  ctx: ProfileCustomJSContext
): Promise<void> => {
  try {
    const extend = typeof module?.extendCustomJSApi === 'function'
      ? module.extendCustomJSApi
...
const extendApiFromProfile = async (
  ...
  const modules = readProfileCustomJSModules();

  for (const module of modules) {
    await extendApiFromProfileModule(module, api, ctx);
  }
};
...
const customJSRuntime = {
  async setup(
    ...
    const setContainerItems = (containerObj: DialogScriptValue, items: DialogScriptValue) => {
      ...
      const likelyCategoricalName = (name: DialogScriptValue) => {
      ...
      flagNames.forEach((flagName) => {
        dataEntry[flagName] = entries.map((entry) => {
          ...
          if (flagName === 'categorical') {
            return entry.enabled !== false
              && (
                entry.flags.categorical === true
                || entry.flags.factor === true
                || entry.flags.character === true
                || entry.flags.logical === true
                || likelyCategoricalName(entry.name)
              );
          }
    ...
    const getSelected = (el: DialogScriptValue) => {
      ...
      if (!obj) return [];
      const t = inferType(obj);
      if (t === 'container') return Array.isArray(obj.value) ? toItems(obj.value) : [];
    ...
    const applyExternalControlUpdate = (value: DialogScriptValue) => {
      if (!value || typeof value !== 'object' || Array.isArray(value)) {
        return;
      }

      const result = value as Record<string, unknown>;
      const controlValues = result.controlValues;
      const controlSelections = result.controlSelections;
      const checkedValues = result.checked;

      if (controlValues && typeof controlValues === 'object' && !Array.isArray(controlValues)) {
        Object.entries(controlValues as Record<string, unknown>).forEach(([name, nextValue]) => {
          setValue(name, nextValue);
        });
      }

      if (controlSelections && typeof controlSelections === 'object' && !Array.isArray(controlSelections)) {
        Object.entries(controlSelections as Record<string, unknown>).forEach(([name, nextSelection]) => {
          setSelected(name, nextSelection);
        });
      }

      if (checkedValues && typeof checkedValues === 'object' && !Array.isArray(checkedValues)) {
        Object.entries(checkedValues as Record<string, unknown>).forEach(([name, nextChecked]) => {
          if (nextChecked) {
            check(name);
          }
          else {
    ...
    const sendToScriptEditor = (command: DialogScriptValue) => {
      ...
      coms.sendTo('main', dialogRuntimeEventChannels.commandUpdate, normalized);
      coms.sendTo('main', scriptEditorEventChannels.insertCode, { code: normalized });
    };
    ...
    const collectObjectBindingVariableContainers = (variables: DialogScriptValue) => {
      const out: string[] = [];
      const addName = (value: DialogScriptValue) => {
      ...
      if (typeof variables === 'string') {
        ...
        Object.values(variables as Record<string, unknown>).forEach((value) => {
          if (Array.isArray(value)) {
    ...
    const normalizeObjectBindingRequest = (request: DialogScriptValue) => {
      ...
      const datasets = coerceName(record.datasets);

      return {
        ...record,
        dialog: String(record.dialog || '').trim(),
        datasets,
    ...
    const bindObjects = (request: DialogScriptValue) => {
      const normalized = normalizeObjectBindingRequest(request);
      const autoRefresh = normalized.autoRefresh !== false;
      const datasetControl = normalized.datasets;
      const variableContainers = collectObjectBindingVariableContainers(normalized.variables);
      const bindingKey = [
        datasetControl,
      ...
      const refreshDatasetObjects = () => {
        setValue(datasetControl, listObjects('datasets'));
      };

      const refreshVariables = () => {
        const selectedDataset = getSelected(datasetControl)[0] || '';
        const variables = selectedDataset ? listColumns(selectedDataset) : [];

        variableContainers.forEach((container) => {
          setValue(container, variables);
        });
      };

      const refreshFromLocal = () => {
        refreshDatasetObjects();
        refreshVariables();
      };
      ...
      const refresh = async () => {
        ...
        try {
          await callMainExternal('bindObjects', normalized);
        } catch {
          // DialogCreator preview and other lightweight hosts can rely on the
    ...
    const getDatasetVariables = async (dataset: DialogScriptValue) => {
      ...
      if (cached && Array.isArray(cached.colnames) && hasDatasetTypeFlags(cached)) {
        ...
        return names.map((name: DialogScriptValue, index: DialogScriptValue) => {
          ...
          return toDialogVariableEntry(entry);
        }).filter(Boolean);
      }
      try {
        if (!coms || typeof coms.invoke !== 'function') throw new TypeError('coms.invoke is not a function');
...
export default customJSRuntime;
