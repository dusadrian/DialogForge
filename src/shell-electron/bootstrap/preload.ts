...
} from "../../base-app/features/files/shellFileDialogIpc";
import {
...
const requireFirstElectronModule = function(
    ...
    for (const candidate of readElectronModuleCandidates(relativePath)) {
        try {
            ...
            lastError = String(record.message || error);
        }
    }
...
const registerElectronConsoleSyntaxLoader = function(syntaxModule?: unknown): void {
    const loader = requireFirstElectronModule(
        "src/shell-electron/console/electronConsoleSyntaxLoader.js",
        "CONSOLE-SYNTAX"
    ) as {
        registerElectronConsoleSyntaxMonacoLoader?: (
            module?: unknown
        ) => void;
    } | null;

    loader?.registerElectronConsoleSyntaxMonacoLoader?.(syntaxModule);
};
...
const loadElectronPageModule = function(
    relativePath: string,
    errorPrefix: string
): void {
    void requireFirstElectronModule(relativePath, errorPrefix);
};
...
const api: DialogForgeApi = {
    loadMainPage: function(): void {
        registerElectronConsoleSyntaxLoader();
        loadElectronPageModule(
            "src/base-app/pages/main.js",
            "MAIN-RENDERER-ERR"
        );
    },
    loadDatasetEditorPage: function(): void {
        loadElectronPageModule(
            "src/base-app/modules/datasetEditorInterface.js",
            "DATASET-EDITOR-ERR"
        );
    },
    loadScriptEditorPage: function(): void {
        registerElectronConsoleSyntaxLoader();
        loadElectronPageModule(
            "src/base-app/modules/scriptEditorInterface.js",
            "SCRIPT-EDITOR-ERR"
        );
    },
    loadSettingsPage: function(): void {
        loadElectronPageModule(
            "src/base-app/pages/settings.js",
            "SETTINGS-ERR"
        );
    },
    loadDialogBuilderPage: function(): void {
    ...
    registerConsoleSyntaxMonacoLoader: function(syntaxModule?: unknown): void {
        registerElectronConsoleSyntaxLoader(syntaxModule);
    },
    ...
    fetchHelpPage: function(url: string) {
    ...
    onTerminalSettingsUpdated: function(callback) {
        onAppEvent(applicationEventChannels.terminalSettingsUpdated, function(payload) {
            callback(
    ...
    onUpdateDownloadProgress: function(callback) {
        onAppEvent(applicationEventChannels.updateDownloadProgress, callback);
    },
