...
import type {
    ParseScriptFunctionOutline,
    ScriptFunctionSymbol
} from "../outline/scriptFunctionOutline";
import {
...
export interface ScriptOutlineControllerOptions {
    ...
    getLineLabel: () => string;
    languageId: string;
    parseFunctionOutline: ParseScriptFunctionOutline;
...
export const createScriptOutlineController = function(
    ...
    const getActiveSymbols = function(): ScriptFunctionSymbol[] {
        ...
        try {
            return options.parseFunctionOutline(active.model);
        } catch {
            return [];
        }
    };
    ...
    const registerDocumentSymbolProvider = function(
        ...
        try {
            monaco.languages.registerDocumentSymbolProvider(options.languageId, {
                provideDocumentSymbols(model: Monaco.editor.ITextModel) {
                    return options.parseFunctionOutline(model).map((symbol) => {
